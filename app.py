...
def classify_audio_clip(clip):
    ...
    classifier = AudioMiniEncoderWithClassifierHead(2, spec_dim = 1,embedding_dim = 512,depth = 5,
                                                    downsample_factor = 4,resnet_blocks = 2,attn_blocks = 4,
                                                    num_attn_heads =4,base_channels = 32,dropout = 0,kernel_size = 5,
    ...
    if uploaded_file is not None:
        if st.button("Analyze Audio"):
            ...
            with col1:
                ...
                result = result.item()
                st.info(f"Result Probaility : {result}")
            ...
            with col3:
                st.info("Disclaimer")
                st.warning("These Classification or detection mechanisms are not always accurate."
                           " They should be considered as a strong signal and not be considered as a ultimate decision maker")
